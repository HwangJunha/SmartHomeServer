...
<body>
	...
	<nav class="navbar navbar-expand-lg bg-secondary text-uppercase fixed-top" id="mainNav">
		...
		<section id="listForm">
		<h2>
					<a style="color: white;" href="DeviceWriteForm.am">장비 등록하기</a>
		</h2>
		</section>
        </div>
...
%>
		...
		for(int i=0;i<articleList.size();i++){
			if(!(articleList.get(i).getState()==999)){ //9999가 아닐때
			%>
			<tr>
				<td><%=articleList.get(i).getName()%></td>
				<td><%=articleList.get(i).getDeviceKind()%></td>
				<td>
					 <a href="DeviceDetail.am?model=<%=articleList.get(i).getModel() %>&page=<%=nowPage %>">
						<%=articleList.get(i).getModel()%>
				</a>
				</td>
				
				<td><%
				if(articleList.get(i).getState()==0){ // 장비가 존재
					%>
					장비 있음
					<% 	
				}else if(articleList.get(i).getState()==1){ //장비가 없음
					%>
					장비 없음				
					<%
				}
	...
	<section id="pageList">
		...
		[이전]&nbsp;
		<%}else{ %>
		<a href="DeviceList.am?page=<%=nowPage-1 %>">[이전]</a>&nbsp;
		<%} %>
		...
		[다음]
		<%}else{ %>
		<a href=DeviceList.am?page=<%=nowPage+1 %>>[다음]</a>
		<%} %>
	</section>
	<%
    }
	...
	{
	%>
	<section id="emptyArea">등록된 장비가 없습니다.</section>
	<%
	}
%>
		</div>
