...
<%
	ArrayList<UserDevice> articleList=(ArrayList<UserDevice>)request.getAttribute("articleList");
...
<body>
	...
	<nav class="navbar navbar-expand-lg bg-secondary text-uppercase fixed-top" id="mainNav">
		...
		<h1 class="masthead-heading text-white text-uppercase mb-2">사용자 장비 목록</h1>
		<section id="listForm">
		<h2>
					<a style="color: white;" href="UserDeviceWriteForm.am">사용자 장비 등록하기</a>
		</h2>
		</section>
        </div>
        
		<div style="margin: 0 auto; border: 1px; width:800px; height:350px; background: #FFFFFF;">
...
%>
	...
	%>
			<tr>
				...
				<td>
					 <a href="UserDeviceDetail.am?model=<%=articleList.get(i).getModel() %>&page=<%=nowPage %>">
	...
	<section id="pageList">
		...
		[이전]&nbsp;
		<%}else{ %>
		<a href="UserDeviceList.am?page=<%=nowPage-1 %>">[이전]</a>&nbsp;
		<%} %>
		...
		[<%=a %>]
		<%}else{ %>
		<a href="UserDeviceList.am?page=<%=a %>">[<%=a %>]
		</a>&nbsp;
		<%} %>
		<%} %>
		...
		[다음]
		<%}else{ %>
		<a href=UserDeviceList.am?page=<%=nowPage+1 %>>[다음]</a>
		<%} %>
	</section>
	<%
    }
	...
	{
	%>
	<section id="emptyArea">등록된 사용자 장비가 없습니다.</section>
	<%
	}
	%>
		</div>
