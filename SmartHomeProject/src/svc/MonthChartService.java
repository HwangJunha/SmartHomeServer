...
public class MonthChartService {
	public ArrayList<DeviceData> ChartService(ArrayList<DeviceData> articleList){ //정렬된 상태의 데이터가 넘어옴 
		...
		String str= articleList.get(0).getTimeYMD().substring(0,6);
		...
		for(int i=0; i<articleList.size()-1; i++) {
			if(str.equals(articleList.get(i+1).getTimeYMD().substring(0,6))) {
				count++; //증가 
				...
				count=0;
				str=articleList.get(i+1).getTimeYMD().substring(0,6); //새로운날짜 
				tempDust=0;
				tempHumidity=0;
				tempTemperature=0;
			}
